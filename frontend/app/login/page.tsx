...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            // Call login API
            await api.login(parseInt(userId));

            // Store user_id in localStorage
            localStorage.setItem("user_id", userId);
            router.push("/");
        } catch (err) {
            setError(err instanceof Error ? err.message : "Login failed. Please check your User ID.");
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <Card className="w-full max-w-md border-2 shadow-2xl">
                <CardHeader className="text-center space-y-4">
                    ...
                    <CardDescription className="text-base">Enter your user ID to discover amazing anime recommendations</CardDescription>
                </CardHeader>
                ...
                <form onSubmit={handleLogin}>
                    ...
                    <CardFooter className="flex flex-col gap-4">
                        ...
                        <div className="pt-4 border-t">
                            <Button variant="outline" type="button" className="w-full bg-transparent" onClick={() => router.push("/admin")}>
                                Admin Portal
                            </Button>
                        </div>
                    </CardFooter>
